...
def _build_json_data(
    ...
    for r in results:
        stats = {k: (None if isinstance(v, float) and math.isnan(v) else v) for k, v in r.stats_dict().items()}
        entry = {
            'name': r.name,
            'category': r.category,
...
def write_csv(
    ...
    with path.open('w', newline='') as f:
        ...
        for r in results:
            ...
            row.update(
                {k: ('' if isinstance(v, float) and math.isnan(v) else v) for k, v in r.stats_dict().items()}
            )
            writer.writerow(row)
