...
def test_dds_response(dap_client):
    ...
    assert ("Float32 air[time = 2920][lat = 25][lon = 53]" in content) or (
        "Float64 air[time = 2920][lat = 25][lon = 53]" in content
    )
...
def test_dods_response(dap_client):
    ...
    # Float32/64 doesn't seem to be consistent across versions. see https://github.com/pydata/xarray/issues/9041
    assert ("Float32 air[time = 2920][lat = 25][lon = 53]" in text_content) or (
        "Float64 air[time = 2920][lat = 25][lon = 53]" in text_content
    )
